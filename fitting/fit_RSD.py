...
sn = power_rsd[0,2]
mall_rsd_withSN = mall_rsd + sn
mall_real_withSN = mall_pos + sn
mall_div_withSN = mall_rsd_withSN/mall_real_withSN

fig, ax = plt.subplots(figsize=(8, 6))
ax.set_xlabel(r'$k = \sqrt{k_x^2 + k_y^2}$')
ax.set_ylabel(r'$k_z$')
ax.set_title(r'$P_{RSD}$ for all galaxies without SN, Npix=64')
ax.grid(visible=True)
cax = ax.imshow(fullGrid(mall_rsd), origin='lower',extent=(-1,1,-1,1),cmap='nipy_spectral',norm=matplotlib.colors.LogNorm(vmin=50,vmax=1.1e5))
fig.colorbar(cax,label=r'$P_{RSD}(k)$')
plt.savefig('fitting/mall_rsd.png')
#plt.show()
plt.close()
...
ax.set_xlabel(r'$k = \sqrt{k_x^2 + k_y^2}$')
ax.set_ylabel(r'$k_z$')
ax.set_title(r'$P_{RSD}$ for all galaxies with SN, Npix=64')
ax.grid(visible=True)
cax = ax.imshow(fullGrid(mall_rsd_withSN), origin='lower',extent=(-1,1,-1,1),cmap='nipy_spectral',norm=matplotlib.colors.LogNorm(vmin=50,vmax=1.1e5))
fig.colorbar(cax,label=r'$P_{RSD}(k)$')
plt.savefig('fitting/mall_rsd_withSN.png')
#plt.show()
plt.close()
...
#Trying to add in additional weights that scale with Shotnoise; RSD-SN / SN * (1-mu**2)**0.5

def weights_comb(grid,weights,shotnoise,mu):
    weights_poisson = weights 
    weights_sn = grid/shotnoise * (1-mu**2)**0.5
    print(weights_poisson)
    print(weights_sn)
    return weights_poisson*weights_sn
...
plt.colorbar()
plt.savefig('fitting/total_weights.png')
plt.close()
...
def func_gaussian(x,mu,kz,power):
    ...
    #Now, compute the sum-squared error between the model and divided power 
    diff_squared = (model-power)**2 
    #return np.sum(weights_total**2 * diff_squared) / np.sum(weights_total)
    ...
    #Extra weight: ratio of shotnoise; multiply weights with this error**2
...
def func_lorentzian(x,mu,kz,power):
    ...
    #As an extra step, apply the weights computed beforehand: 
    #return np.sum(weights * np.abs(model-power)/np.sum(weights))
    #return np.sum(weights_total**2 * (model-power)**2)
    diff_sqr_weighted = weights_total**2 * (model-power)**2 / np.sum(weights_total)
...
result_gaussian = minimize(func_gaussian,[0.5,1],args=(mu_mall64,Y,mall_div),method='L-BFGS-B',bounds=((0,1),(0,10)))
...
result_gaussianSN = minimize(func_gaussian,[0.5,1],args=(mu_mall64,Y,mall_div_withSN),method='L-BFGS-B',bounds=((0,1),(0,10)))

print('Using L-BFGS-B minimization and a Gaussian profile, but using the non-Shotnoise subtracted real and RSD spectra')
print(result_gaussianSN.x)

result_lorentzianSN = minimize(func_lorentzian,[0.5,1],args=(mu_mall64,Y,mall_div_withSN),method='L-BFGS-B',bounds=((0,1),(0,10)))

print('Using L-BFGS-B minimization and a Lorentzian profile, but using the non-Shotnoise subtracted real and RSD spectra')
print(result_lorentzianSN.x)
...
mall_div_lorentzian_withSN = model_lorenztian(result_lorentzianSN.x,mu_mall64,Y)
...
print('The mean and median of the velocity dispersions in the z direction are:')
print(np.mean(mall_disp_z**0.5))
print(np.median(mall_disp_z**0.5))

_, bins = np.histogram(np.log10((mall_disp_z**0.5) + 1), bins='auto')
plt.hist((mall_disp_z**0.5), bins=10**bins)
plt.gca().set_xscale("log")
plt.xlabel('velocity dispersion (sigma_v)')
plt.ylabel('N')
plt.show()
...
def plotFit(ydata,model,name):

    residual,residual_weighted = residuals(ydata,model)

    fig, ax = plt.subplots(2,2,figsize=(10,10))
    cax1 = ax[0,0].imshow(ydata, origin='lower',extent=(0,1,0,1),cmap='nipy_spectral',vmin=0.5,vmax=2.5)
    ...
    cax3 = ax[1,0].imshow(residual, origin='lower',extent=(0,1,0,1),cmap='nipy_spectral',vmin=0,vmax=2)
    ...
    cax4 = ax[1,1].imshow(residual_weighted, origin='lower',extent=(0,1,0,1),cmap='nipy_spectral',vmin=0,vmax=0.005)
    ...
    fig.suptitle('Comparison of true and modelled RSD/Real grid ({})'.format(name))
    fig.savefig('fitting/{}.png'.format(name))

    #plt.tight_layout()
    #plt.show()
    plt.close()

plotFit(mall_div,mall_div_model,'Gaussian')
plotFit(mall_div,mall_div_lorentzian,'Lorentzian')
plotFit(mall_div,mall_div_lorentzian_basic,'Lorentzian_basic')
plotFit(mall_div,mall_div_lorentzian_withSN,'Lorentzian_withSN')