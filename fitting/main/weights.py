...
def weights_grid(variance,p):
    ...
    for i in range(N):
        for j in range(N):
            ...
            pxl_section = grid_variance[rowidx_start:rowidx_stop,colidx_start:colidx_stop]
...
fig, ax = plt.subplots(figsize=(10,10))
cax1 = ax.imshow(1/np.sqrt(smoothed_variance), origin='lower',extent=(0,1,0,1),cmap='nipy_spectral',vmin=1,vmax=40)
ax.set_xlabel(r'$k = \sqrt{k_x^2 + k_y^2}$')
ax.set_ylabel(r'$k_z$')
