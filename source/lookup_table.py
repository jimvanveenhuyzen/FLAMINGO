...
#We get the datafile from the directory specified in properties.py
with h5py.File('/disks/cosmodm/vanveenhuyzen/HYDRO_STRONGEST_AGN/sp_STRONGEST_AGN.h5', 'r') as data:
    ...
    GroupNr_all = np.array(GroupNr_all,dtype=np.int64) #Convert to a numpy array to perform mathematical operations
data.close()
...
#Initialize an empty list and index_start,index_prev to keep track of indices 
lookup = []
index_start = 0
index_prev = 0
...
np.save('STRONGEST_AGN/GroupNr_table_STRONGEST_AGN.npy',lookup)